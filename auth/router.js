...
// Usuario autenticado + su perfil (rol, vínculo GHL).
router.get('/me', requireAuth, async (req, res) => {
  const prof = await getProfile(req.user.id);
  res.json({ id: req.user.id, email: req.user.email, profile: prof || null });
});
...
router.post('/users', requireAuth, requireAdmin, async (req, res) => {
  ...
  const { error: perr } = await admin.from('profiles').upsert({
    id, email, role, full_name: b.fullName || null, ghl_user_id: b.ghlUserId || null
  });
